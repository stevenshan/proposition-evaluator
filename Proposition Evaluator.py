...
# 			evaluate (formula, vars)
		...
		for x in self._formulas:
			n = len(x)
			m = (n + proposition._row_padding * 2) if n + proposition._row_padding * 2 >= proposition._minimum_row_width else proposition._minimum_row_width
			l = (m - n) / 2.0
			lengths.append(m)
	...
	class logic:
		...
		@staticmethod
		def _xor(p1, p2):
			return True if ((p1 is True and p2 is False) or (p1 is False and p2 is True)) else False
		@staticmethod
		def _implies (p1, p2):
	...
	operations_names = ["#not", "and", "or", "xor", "implies", "biimplies", "nand"]
	...
	def split (formula, negate = False):
		...
		while i < len(formula):
			...
			if unit == "(":
				...
				if parenthesis_index == 0:
					result.extend(proposition.split(sub, (True if not_found == 2 else False)))
					not_found = 0
					sub = ""
		...
		while i < len(result) - 1:
			j = i + 1
			while j < len(result):
				if result[i] == result[j] or re.match("^\([ ]*" + re.escape(result[i]) + "[ ]*\)$", result[j]):
					del result[j]
					break
	...
	def remove_duplicate_parenthesis (formula):
		...
		for i in range(0, len(formula)):
			unit = formula[i]
			if unit == "(":
				if found == True and forward == True:
					dio.append(index)
				index = i
				found = True
				forward = True
...
# 		print(" ".join([i + " " for i in formula])
	...
	def format_strict (formula):
		map = {}
		add = lambda x : proposition.format_add(map, x)
		strip = lambda x: proposition.strip_outer_parenthesis(x)
		while formula.find("(") != -1:
			...
			while i < len(formula):
				...
				if unit == "(":
					...
					terp = formula
					formula = formula[0:start] + add(strip(proposition.format_strict(sub))) + formula[i+1:]
		...
		for op in proposition.operations_names:
			if op[0] == "#":
				...
				while flag is True:
					...
					while i < len(formula):
						temp_flag = False
						if i + len(op) < len(formula) and formula[i:i+(len(op))+1] == op + " ":
							if flag is True:
								temp_flag = True
								flag = False
							j = i + len(op)
							...
							start = 0
							while j < len(formula) and (not space_found or not alphanum_found):
								if re.match("^[a-z0-9]$", formula[j]) and not alphanum_found:
									alphanum_found = True
									start = j
								elif alphanum_found and formula[j] == " ":
									if ("#" + formula[start:j]) in proposition.operations_names:
										i = start - 1
										flag = True
										break
									space_found = True
								...
								j += 1
							if not flag:
								formula = formula[0:i] + add(formula[i:j]) + formula[j:]
							if temp_flag:
								flag = True
						i += 1
			else:
				while formula.find(" " + op + " ") != -1:
					i = len(formula) - 1
					while i >= 0:
						if i - len(op) - 1 >= 0 and formula[i-len(op)-1:i+1] == " " + op + " ":
							j = i + 1
							space_found = False
							alphanum_found = False
							while j < len(formula) and (not space_found or not alphanum_found):
							...
							space_found = False
							alphanum_found = False
							while j >= 0 and (not space_found or not alphanum_found):
								if re.match("^[a-z0-9]$", formula[j]) and not alphanum_found:
									alphanum_found = True
								elif alphanum_found and formula[j] == " ":
									space_found = True
								...
								j -= 1
							start = j
							formula = formula[0:start+1] + add(formula[start+1:end]) + formula[end:]
						i -= 1
		generate_list = lambda: [x for x in re.findall("[0-9]+", formula) if x in map.keys()]
		list = generate_list()
		find_map = lambda x: ("( " + str(map[x]) + " )") if re.match("^[0-9]+$", x) and x in map else x
		while len(list) > 0:
	...
	def evaluate (formula, vars):
		...
		while i != -1:
			...
			e = "TRUE" if proposition.evaluate_connective(cmd, p1, p2) is True else "FALSE"
